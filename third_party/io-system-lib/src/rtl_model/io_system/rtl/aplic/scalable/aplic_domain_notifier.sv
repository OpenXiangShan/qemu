...
module aplic_domain_notifier #(
    ...
    input   logic [63:0]                            i_imsic_addr_target,
    `ifdef DIRECT_MODE
...
`elsif MSI_MODE
    ...
    logic [63:0]               imsic_addr_target_i;

    assign imsic_addr_target_i = (i_imsic_addr_target != 64'h0) ?
                                 i_imsic_addr_target : IMSIC_ADDR_TARGET;
    ...
    always_comb begin : find_highest_pen_en
        ...
        for (int i = 1 ; i < NR_SRC ; i++) begin
            ...
            if (i_setip_q[i/32][i%32] && i_setie_q[i/32][i%32] && i_domaincfgIE && !axi_busy_q) begin
                ...
                addr_i          = imsic_addr_target_i + ({{64-32{1'b0}}, i_target_q[i]} & TARGET_GUEST_IDX_MASK);
                ready_i         = 1'b1;
            end
        end
        ...
        if (i_genmsi[12] && !axi_busy_q) begin
            ...
            addr_i          = imsic_addr_target_i + ({{64-32{1'b0}}, i_target_q[i_genmsi[10:0]]} & TARGET_GUEST_IDX_MASK);
            genmsi_sent     = 1'b1;
            ready_i         = 1'b1;
        end
    end
