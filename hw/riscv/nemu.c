...
//                       base_output_path);
        ...
        s->path_manager.checkpoint_path_list =
            g_list_sort_with_data(s->path_manager.checkpoint_path_list,
                                  g_compare_path, NULL);

        s->simpoint_info.cpt_instructions =
            g_list_sort_with_data(s->simpoint_info.cpt_instructions,
                                  g_compare_instrs, NULL);
...
static void nemu_load_firmware(MachineState *machine)
{
    ...
    hwaddr firmware_load_addr = memmap[NEMU_DRAM].base;
    ...
    firmware_end_addr = riscv_find_and_load_firmware(
        machine, &boot_info, riscv_default_firmware_name(&s->soc[0]),
...
static void nemu_machine_class_init(ObjectClass *oc, const void *data)
{
