...
static void try_profiling(unsigned int cpu_index, void *userdata) {
  if (cpu_index != 0) {
    return;
  }
  // hash key
...
static void prepare_bbl(struct qemu_plugin_tb *tb, void *userdata) {
  (void)userdata;
...
QEMU_PLUGIN_EXPORT int qemu_plugin_install(qemu_plugin_id_t id,
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (g_strcmp0(tokens[0], "workload") == 0) {
      ...
      g_strlcpy(profiling_info.args.worklaod_path, tokens[1], FILENAME_MXLEN);
      ...
      g_strlcpy(profiling_info.args.target_path, tokens[1], FILENAME_MXLEN);
  ...
  qemu_plugin_register_vcpu_tb_trans_cb(id, prepare_bbl, NULL);
